 ...
 * @param {object} tokens - The tokens to track the balance changes for
 ...
 * @returns BigInt representing the change of the KTA balance after the target date
 */
async function tokenBalanceChangesAfter(account, targetDate, tokens, currentHeadBlock) {
  ...
  while (!timestampReached) {
    ...
    for (const { voteStaple } of history.history) {
      ...
      // If any of the blocks in the staple were created before the targetDate,
      // ignore the entire staple.
      const isBeforeTargetDate = voteStaple.blocks.find(block => {
      ...
      for (const block of voteStaple.blocks) {
        for (const operation of block.operations) {
          ...
          if (operation.type === 7 && balanceChanges.has(operation.token) && operation.forward === account) {
            if (operation.exact) {
              balanceChanges[operation.token] += BigInt(operation.amount);
            } else {
...
/**
 * Gets the KTA balance of an address at the given timestamp on the Keeta mainnet.
 *
